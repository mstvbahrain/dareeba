...
const statements = [
  `DO $$ BEGIN CREATE TYPE "ReportStatus" AS ENUM ('PREVIEW', 'PAID', 'REFERRED'); EXCEPTION WHEN duplicate_object THEN null; END $$`,
  `DO $$ BEGIN CREATE TYPE "RiskLevel" AS ENUM ('LOW', 'MEDIUM', 'HIGH'); EXCEPTION WHEN duplicate_object THEN null; END $$`,
  `DO $$ BEGIN CREATE TYPE "VatTreatment" AS ENUM ('STANDARD_RATED', 'ZERO_RATED', 'EXEMPT', 'OUTSIDE_SCOPE', 'NEEDS_REVIEW'); EXCEPTION WHEN duplicate_object THEN null; END $$`,
  `DO $$ BEGIN CREATE TYPE "PaymentStatus" AS ENUM ('PENDING', 'PAID', 'FAILED'); EXCEPTION WHEN duplicate_object THEN null; END $$`,
  `DO $$ BEGIN CREATE TYPE "LeadStatus" AS ENUM ('NEW', 'SENT_TO_PARTNER', 'CONTACTED', 'CONVERTED', 'COMMISSION_RECEIVED'); EXCEPTION WHEN duplicate_object THEN null; END $$`,
...
export async function GET() {
  ...
  return NextResponse.json({
    ok: true,
    message: "Dareeba database tables and seed data are ready.",
    pricingPlans,
    vatRules
  });
}
